...
1. v1/PRD에 포함되어 있던 실제 점검 항목을 v2에 반영한다.
   ...
   - KISA 「주요정보통신기반시설 기술적 취약점 분석·평가 상세 가이드」 기반 웹서비스 점검 항목 26개: WEB-01~WEB-26 (`web 점검 컴플라이언스.MD` 기준 — 이전 판(v2 Revised)의 W-01~W-27/Apache·IIS 분리 표는 오류였으며 이 문서로 대체됨)
   - 총 카탈로그 기준 102개 항목
...
> **완주 안전장치:** 전체 카탈로그는 102개 항목으로 구성하되, 먼저 C-01, C-02, U-16 세 항목으로 파이프라인을 끝까지 배선한다. 이후 C 전체 → U 계정/파일권한 → U 서비스/패치/로그 → WEB 웹서비스 순서로 확장한다.
...
| KISA 주요정보통신기반시설 기술적 취약점 분석·평가 상세 가이드 기반 웹서비스 점검 항목  | WEB-01~WEB-26 |      26 | 카탈로그 전체 포함, nginx 매핑 가능 항목부터 자동화 |
| 총합                                                                                    | C + U + WEB |     102 | 자동화 전 항목은`not_automated` 또는 `skip`으로 분리  |
...
### 5.5 KISA 주요정보통신기반시설 기술적 취약점 분석·평가 상세 가이드 기반 웹서비스 점검 항목 — WEB-01~WEB-26

> 출처: `web 점검 컴플라이언스.MD` (KISA 『주요정보통신기반시설 기술적 취약점 분석·평가 방법 상세가이드』 Chapter 03. 웹 서비스). 이전 판(v2 Revised)에 있던 W-01~W-27(Apache/IIS 27개 분리 표)은 실제 가이드와 불일치하는 오류였으며, 이 절로 전면 대체한다.

컨테이너가 웹서버(주로 nginx)를 포함할 때만 적용한다. 이 가이드는 Apache/IIS로 항목을 나누지 않고 전부 서버 중립적으로 서술하므로, "IIS라서 스킵" 같은 플랫폼 분기 없이 각 항목을 nginx 설정으로 판정 가능한지만 기준으로 삼는다. nginx로 합리적으로 판정 가능한 항목만 자동화하고(현재 14개: WEB-03/04/08/09/13/16/18/19/20/21/22/23/25/26), 관리자 계정·비밀번호 정책·CGI 제한처럼 애플리케이션/운영 정책에 의존하는 항목은 `not_automated`로 남긴다. 웹서버가 감지되지 않으면 전체가 `skip`, 정적 스냅숏만으로 판단할 수 없는 항목(버전 기반 패치 점검 등)은 `review`로 분류한다.

| ID     | 심각도 | 항목                                        | MVP 자동화 |
| ------ | :----: | ------------------------------------------- | :--------: |
| WEB-01 |   상   | Default 관리자 계정명 변경                  | not_automated |
| WEB-02 |   상   | 취약한 비밀번호 사용 제한                   | not_automated |
| WEB-03 |   상   | 비밀번호 파일 권한 관리                     | 자동화 |
| WEB-04 |   상   | 웹 서비스 디렉터리 리스팅 방지 설정         | 자동화 |
| WEB-05 |   상   | 지정하지 않은 CGI/ISAPI 실행 제한           | not_automated |
| WEB-06 |   상   | 웹 서비스 상위 디렉터리 접근 제한 설정      | not_automated |
| WEB-07 |   중   | 웹 서비스 경로 내 불필요한 파일 제거        | not_automated |
| WEB-08 |   하   | 웹 서비스 파일 업로드 및 다운로드 용량 제한 | 자동화 |
| WEB-09 |   상   | 웹 서비스 프로세스 권한 제한                | 자동화 |
| WEB-10 |   상   | 불필요한 프록시 설정 제한                   | not_automated |
| WEB-11 |   중   | 웹 서비스 경로 설정                         | not_automated |
| WEB-12 |   중   | 웹 서비스 링크 사용 금지                    | not_automated |
| WEB-13 |   상   | 웹 서비스 설정 파일 노출 제한               | 자동화 |
| WEB-14 |   상   | 웹 서비스 경로 내 파일의 접근 통제          | not_automated |
| WEB-15 |   상   | 웹 서비스의 불필요한 스크립트 매핑 제거     | not_automated |
| WEB-16 |   중   | 웹 서비스 헤더 정보 노출 제한               | 자동화 |
| WEB-17 |   중   | 웹 서비스 가상 디렉토리 삭제                | not_automated |
| WEB-18 |   상   | 웹 서비스 WebDAV 비활성화                   | 자동화 |
| WEB-19 |   중   | 웹 서비스 SSI(Server Side Includes) 사용 제한 | 자동화 |
| WEB-20 |   상   | SSL/TLS 활성화                              | 자동화 |
| WEB-21 |   중   | HTTP 리디렉션                               | 자동화 |
| WEB-22 |   하   | 에러 페이지 관리                            | 자동화 |
| WEB-23 |   중   | LDAP 알고리즘 적절하게 구성                 | 자동화 |
| WEB-24 |   중   | 별도의 업로드 경로 사용 및 권한 설정        | not_automated |
| WEB-25 |   상   | 주기적 보안 패치 및 벤더 권고사항 적용      | 자동화 (review) |
| WEB-26 |   중   | 로그 디렉터리 및 파일 권한 설정             | 자동화 |
...
- KISA PDF runtime 파싱
- 외부 사이트 scraping
- U-01~U-67, WEB-01~WEB-26 전체 항목의 완전 자동화
- Windows OS 자체 점검
- DB 이미지 보안 점검
...
2. 심각도 대시보드 차트
3. nginx 웹 항목 WEB-03, WEB-04, WEB-08, WEB-09, WEB-13, WEB-16, WEB-18, WEB-19, WEB-20, WEB-21, WEB-22, WEB-23, WEB-25, WEB-26 자동화 (완료)
4. httpd/tomcat 지원
...
| 리스크                             | 설명                                                           | 완화책                                                |
| ---------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------- |
| 점검 항목 과다                     | 총 102개 항목으로 범위가 큼                                    | C-01·C-02·U-16으로 E2E 먼저 완성                    |
