...
| KISA 주요정보통신기반시설 기술적 취약점 분석·평가 상세 가이드 기반 웹서비스 점검 항목  | WEB-01~WEB-26 |      26 | 전체 자동화 (nginx 기준) |
...
컨테이너가 웹서버(주로 nginx)를 포함할 때만 적용한다. 이 가이드는 Apache/IIS로 항목을 나누지 않고 전부 서버 중립적으로 서술하므로, "IIS라서 스킵" 같은 플랫폼 분기 없이 각 항목을 nginx 설정으로 판정 가능한지만 기준으로 삼는다. WEB-01~WEB-26 전체 26개 항목을 nginx 설정/실행 컨테이너 기준으로 자동화했다. 웹서버가 감지되지 않으면 전체가 `skip`이고, 계정명·비밀번호 강도처럼 htpasswd 해시 내부라 설정만으로 확인 불가능한 항목과 정적 스냅숏만으로 최신성을 판단할 수 없는 패치 점검은 `review`로, "가상 디렉토리 필요 여부"처럼 존재 자체는 확인되지만 필요성 판단에 사람 개입이 필요한 항목도 `review`로 분류한다.

| ID     | 심각도 | 항목                                        | 판정 방식 |
| ------ | :----: | ------------------------------------------- | :--------: |
| WEB-01 |   상   | Default 관리자 계정명 변경                  | 자동화 (auth_basic 없으면 skip, 있으면 review) |
| WEB-02 |   상   | 취약한 비밀번호 사용 제한                   | 자동화 (auth_basic 없으면 skip, 있으면 review) |
| WEB-03 |   상   | 비밀번호 파일 권한 관리                     | 자동화 |
| WEB-04 |   상   | 웹 서비스 디렉터리 리스팅 방지 설정         | 자동화 |
| WEB-05 |   상   | 지정하지 않은 CGI/ISAPI 실행 제한           | 자동화 |
| WEB-06 |   상   | 웹 서비스 상위 디렉터리 접근 제한 설정      | 자동화 (nginx off-by-slash alias 오탐지) |
| WEB-07 |   중   | 웹 서비스 경로 내 불필요한 파일 제거        | 자동화 |
| WEB-08 |   하   | 웹 서비스 파일 업로드 및 다운로드 용량 제한 | 자동화 |
| WEB-09 |   상   | 웹 서비스 프로세스 권한 제한                | 자동화 |
| WEB-10 |   상   | 불필요한 프록시 설정 제한                   | 자동화 (proxy_pass SSRF 패턴 탐지) |
| WEB-11 |   중   | 웹 서비스 경로 설정                         | 자동화 (root가 시스템 경로인지 판정) |
| WEB-12 |   중   | 웹 서비스 링크 사용 금지                    | 자동화 |
| WEB-13 |   상   | 웹 서비스 설정 파일 노출 제한               | 자동화 |
| WEB-14 |   상   | 웹 서비스 경로 내 파일의 접근 통제          | 자동화 |
| WEB-15 |   상   | 웹 서비스의 불필요한 스크립트 매핑 제거     | 자동화 (2개 이상 매핑 시 review) |
| WEB-16 |   중   | 웹 서비스 헤더 정보 노출 제한               | 자동화 |
| WEB-17 |   중   | 웹 서비스 가상 디렉토리 삭제                | 자동화 (alias 존재 시 review) |
...
| WEB-24 |   중   | 별도의 업로드 경로 사용 및 권한 설정        | 자동화 |
...
2. 심각도 대시보드 차트
3. nginx 웹 항목 WEB-01~WEB-26 전체 26개 자동화 (완료)
4. httpd/tomcat 지원
