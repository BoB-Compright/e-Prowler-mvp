...
**사내망 밖 PM에게 공유하기 (점검자별 임시 터널)** — 공유 링크는 별도 설정 없이 **점검자가 접속 중인 주소를 그대로** 따릅니다. 사내망 PM에게는 사내 IP로 접속한 상태에서 공유하면 되고, 인터넷의 PM에게는 Cloudflare 임시 터널을 열면 됩니다:

```bash
npm run start     # NH-Guardian 서버 기동
npm run tunnel    # 임시 공개 URL 발급 (https://<랜덤>.trycloudflare.com)
```

점검자가 발급된 터널 URL로 접속해 로그인한 뒤 공유(복사·QR·메일)하면, 링크가 자동으로 그 터널 주소로 만들어집니다. 계정·도메인 등록이 필요 없고, **점검자마다 자기 터널 = 자기 링크**라 여러 명이 동시에 각자의 링크로 공유할 수 있습니다. 터널을 끄면(Ctrl+C) 외부 접근은 즉시 끊기며, URL은 실행할 때마다 바뀝니다. 터널이 열려 있는 동안 앱이 인터넷에 노출되므로(로그인·공유 비밀번호로 보호) 관리자 비밀번호를 강하게 유지하고 공유가 끝나면 터널을 내리세요.
