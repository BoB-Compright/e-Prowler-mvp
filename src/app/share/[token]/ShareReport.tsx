...
// ReportView.tsx의 SEVERITY_STATUS와 동일하게 유지한다.
...
// 룰이 판정을 보류(검토)한 항목을 AI가 판정했음을 표시하는 아이콘.
// ReportView.tsx의 ClaudeSparkleIcon과 동일한 SVG를 여기서도 로컬로 둔다.
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          ...
          <p className="mt-1 text-[13px] text-muted">
            <span className="font-semibold text-text">{assetName}</span>
            <span className="mx-1.5">·</span>
            <span className="font-mono">{targetLabel}</span>
            <span className="mx-1.5">·</span>
            <span className="font-mono">{scannedAt}</span>
          </p>
        </div>
      </div>
            ...
            <div className="border-b border-border p-3.5">
              ...
              {visibleFrameworks.length > 1 && (
                <div className="mt-1.5 flex flex-wrap gap-1.5">
                  <button className={chipStyle(!frameworkFilter)} onClick={() => setFrameworkFilter(null)}>
                    전체
                  </button>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {check.severity && (
                        <span className="ml-auto">
                          <StatusBadge status={SEVERITY_STATUS[check.severity]}>{check.severity}</StatusBadge>
                        </span>
                      )}
                    </div>
          ...
          {selected && (
            <Card className="min-w-[360px] flex-[2]">
              <div className="font-mono text-[11px] text-muted">
                {assetName} / {CATEGORY_CHIP_LABELS[selected.category ?? "container"]}
              </div>
              <div className="mt-2 flex flex-wrap items-center gap-2.5">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${SOURCE_STYLES[selected.source]}`}>
              ...
              <div className="mt-5">
                ...
                <p className="mt-1.5 text-sm leading-relaxed">
                  {selected.reason ? <InlineCodeText text={selected.reason} /> : "아직 분석되지 않았습니다."}
                </p>
              </div>
