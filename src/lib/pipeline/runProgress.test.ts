...
describe("runProgress", () => {
  it("컨테이너(git) 경로: 6단계 순번/6", () => {
    expect(runProgress({ stage: "clone", sourceType: "git" })).toEqual({ label: "소스 가져오기", fraction: 1 / 6 });
    expect(runProgress({ stage: "build", sourceType: "git" })).toEqual({ label: "이미지 빌드", fraction: 2 / 6 });
    expect(runProgress({ stage: "claude", sourceType: "git" })).toEqual({ label: "AI 심층 분석", fraction: 1 });
  });
  ...
  it("서버 경로: 4단계", () => {
    expect(runProgress({ stage: "connect", sourceType: "server" })).toEqual({ label: "서버 연결", fraction: 1 / 4 });
  ...
  it("경로에 없는 stage는 라벨만 출력하고 fraction 0 (방어)", () => {
    expect(runProgress({ stage: "clone", sourceType: "server" })).toEqual({ label: "소스 가져오기", fraction: 0 });
  });
});
