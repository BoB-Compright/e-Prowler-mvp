...
`src/lib/assets/store.ts`의 `deleteAsset` 트랜잭션에 한 줄 추가:
...
`src/app/runs/page.tsx`의 테이블 헤더 `<th ...>상태</th>` 앞에 추가:
...
```tsx
<td className="px-2.5 py-2.5 text-center text-[11.5px] text-[var(--color-muted)]">
