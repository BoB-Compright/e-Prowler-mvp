...
|---|---|---|
| WAS-01 기본/샘플 앱 제거 | pass/fail | webapps에 manager/host-manager/examples/docs 잔존 → 취약 (ROOT는 모든 설치에 기본 존재하므로 제외 — 노이즈 방지) |
...
| WAS-10 TRACE 메서드 비활성화 | pass/fail | HTTP Connector `allowTrace="true"` → 취약 (TLS 유무는 별도 항목 후보, 이번엔 allowTrace만 판정) |
