...
- 단, `Dockerfile.<suffix>` / `<prefix>.Dockerfile` 변형은 확장자(마지막 `.` 기준, 대소문자 무시)가
  흔한 비-Dockerfile 확장자 데니리스트(`.ts .js .jsx .tsx .py .rb .go .rs .java .md .txt .png .jpg
  .jpeg .gif .svg .yml .yaml .json .sh .lock .log .html .css`)에 있으면 제외한다
  (예: `Dockerfile.md`, `dockerfile.ts` 오탐 방지). `app.Dockerfile`의 확장자는 `.dockerfile`이라
  데니리스트에 없으므로 그대로 매치된다. 정확히 `Dockerfile`인 경우는 데니리스트와 무관하게 항상 매치.
...
```
buildImage(dockerfilePath: string, imageTag: string): Promise<void>
```

실행: `docker build -t <imageTag> -f <dockerfilePath> <contextDir>`

**컨텍스트 = Dockerfile이 있는 디렉터리 (루트 Dockerfile이면 repoDir와 동일).** 근거:
- 자기완결형 하위 서비스 Dockerfile(`backend/Dockerfile`의 `COPY requirements.txt ./` 등)이
  하위 디렉터리 파일을 정상적으로 찾을 수 있다.
- 루트 Dockerfile은 `dirname(dockerfilePath) === repoDir`이라 기존 동작과 동일하게 보존된다.
...
- `undefined`면 실패 메시지를 `"Dockerfile을 찾을 수 없습니다 (레포 전체 탐색)"`로 갱신
  (더 이상 루트 한정이 아님).
- build 호출을 `deps.build(dockerfilePath, imageTag)`로 변경
